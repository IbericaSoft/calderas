<?	
	...
	*	@version 5.0 (12/11)
	*		Cambio radical. Se singletoniza la clase y se repasa todo el funcionamiento cacheando la navegaci�n. Se termina de integrar el sistema
	*		de log.
	*	@version 4.0 (5/11)
	*		Los errores se notifican por email
	*		Multi-idioma (no funcionaba bien)
	*		Cambiamos la forma de usar la clase. Primero se instancia, despu�s se pasa el objeto conexi�n a datos, despues podemos cachear los objetos que queramos y finalmente lanzamos la ejecucion
	*	@version 3.1 (10/5/09) 
	*		Se habilita la posibilidad de ejecuci�n de metodos estandard definidos en acciones con el prefijo '$', es decir, que independientemente de la acci�n solicitada
	*		se ejecutaran todas las acciones con el prefijo indicado. No son acciones encadenables.
	*	@version 3.0 (16/9/08) Las acciones con "PRE ACCION" ahora se evaluan con true o false, y llevan a una navegaci�n nueva, antes solo ocurr�a en caso false. Este tipo
	*		de navegaci�n es ahora como un IF para encadenar condiciones si es necesario
	...
	*		El sistema permite previa evaluaci�n del metodo de verificaci�n de las acciones, cambiar el flujo de navegaci�n volviendo a emprezar.
	*	@Version 2.0 (26/02/2008)
	*		En la versi�n anterior, un archivo plantilla estaba formado por piezas. Esas piezas llamaban un metodo getPiezas(xxx) para generar los datos de la misma. Pero
	...
	*	determinar si esta accion ha de llegar a fin o el sistema cambia el flujo de navegaci�n, volviendo a empezar.
	*/	
	...
	class Navigator {
		...
		const SQL_NAVITATORDETAIL 	= 'SELECT * FROM _navigator_detail WHERE estado!="XXX" ORDER BY orden;';
		...
		/**
		 * Nos sirve de punto similar al constructor al recuperar la instancia cacheada, adem�s
		...
		private function notifyRecover(){
			...
			if ( !self::PRODUCTION_MODE ){
				//self::$log->debug( "No estamos en modo producci�n");
		...
		/**
		 * El framework tiene que buscar en una tabla la navegaci�n requeriada por el usuario,
		...
		/**
		 * Es una subcarpeta de FolderTemplate y es donde estar�n las plantillas a cargar del pa�s en cuesti�n
		 ...
		 * traducidades del pais indicado. Si no se indica por defecto cargar� el valor default
		 */
		 ...
		 * Si no se indica por defecto cargar� el valor default
		 */
		...
		/**
		 * Acci�n/operaci�n requerida por el usuario
		 * @param $action valor reflejado en BD que identifica 
		 * el fichero PHP y funci�n a invocar por este Framework
		 */
		...
		/**
		 * Devuelve la acci�n/operaci�n requerida por usuario.
		 * Si no se indica por defecto cargar� el valor default
		 */
		 ...
		 * es una navegación desconocida para el sistema
		 */
		 ...
		 * Estas se ejecutan siempre, en cada peticion de navegaci�n del usuario
		 */
		private function autoStartActions(){
			foreach( array_keys($this->dataActions) as $action){
				...
				if ( !function_exists( $metodo ) )
					throw new Exception("La operaci�n ::$action hace referencia al metodo $metodo() pero no lo hemos encontrado!!!");
		...
		/**
		 * Ejecuta la operaci�n solicitada por el usuario, lanzando los metodos necesarios, cargando las plantilas, etc.
		 * Aqui cargamos la plantilla "esqueleto" de la pagina que ver� el usuario
		 */
		private function executeAction(){
			 ...
			 * navegaci�n por otro sitio, es como una action que selecciona otras actions. Hay dos posibilidades, el action para el resultado
			...
			if ( $this->dataActions[$this->getAction()]['preaction'] ) {				
				...
				if ( !function_exists( $metodo ) )
					throw new Exception("La operaci�n ::".$this->getAction()." hace referencia al metodo $metodo() pero no lo hemos encontrado!!!");
		...
		/**
		 * La plantilla pide bloques (peque�os html) que son generados a partir de los PHP de la carpeta workflow
		...
		private function executePackage(){
			...
			if ( $this->block['paquete'] ){
				...
				if ( !function_exists( $metodo ) )
					throw new Exception("La operaci�n ::".$this->getAction()." en su bloque ".$this->block['bloque']." hace referencia al metodo ".$this->block['paquete']." pero no lo hemos encontrado!!!");
		...
		public static function addData($key,$datos){
			self::$log->debug( "Añadiendo datos en $key" );
		...
		/**
		 * Inicia la navegaci�n del usuario cargando las funciones del workflow, plantillas
		 * del idioma, buscando la acci�n requeridad por el usuario, etc.
		 */
		public function execute(){
			try {
				...
				//self::$log->debug("Inicio el proceso de ejecuci�n de la action");
				$this->executeAction();
				//self::$log->debug("Finalizo el proceso de ejecuci�n de la action");
				
				//self::$log->debug("Inicio el proceso de ejecuci�n de los bloques de la plantilla");
				$this->paint();
				//self::$log->debug("Finalizo el proceso de ejecuci�n de los bloques de la plantilla");
				
			} catch (Exception $e){
				self::$log->error( "Se ha producido un error con $_SERVER[REQUEST_URI]: $e");
				echo $e->getMessage();
		 ...
		 * que ir�a ah�. Son por ejemplo, BODY, LEFT, RIGHT, HEAD, BOTTOM, etc. 
		 */
